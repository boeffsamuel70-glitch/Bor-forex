...
# Se o dado da Twelve Data estiver mais atrasado que isso,
...
# Proteção contra sinais Telegram duplicados.
...
def pullback_call_na_vela(
    ...
    # Para CALL procuramos uma correção para baixo.
    # O candle precisa tocar ou atravessar EMA13/EMA21
    # usando principalmente a mínima.
    tocou_ema13 = (
        info["low"] <= ema13
        and info["high"] >= ema13
    )
...
def pullback_put_na_vela(
    ...
    # Para PUT procuramos uma correção para cima.
    tocou_ema13 = (
        info["low"] <= ema13
        and info["high"] >= ema13
    )
...
# ============================================================
# ESTRATÉGIA PULLBACK + CONFIRMAÇÃO
# ============================================================

def analisar_pullback(candles):

    """
    ESTRATÉGIA EM 2 VELAS SEPARADAS.

    VELA -3 / VELA -2:
        Detecta o PULLBACK.

    VELA -1:
        É obrigatoriamente a VELA DE CONFIRMAÇÃO.

    A VELA -1 NÃO é utilizada para detectar o pullback.

    Depois que a vela -1 fecha:
        -> gera CALL ou PUT
        -> entrada pretendida na próxima vela de 5 minutos.
    """
    ...
    # IMPORTANTÍSSIMO:
    #
    # A VELA -1 É A CONFIRMAÇÃO.
    #
    # O PULLBACK SÓ É PROCURADO NAS VELAS -2 E -3.
    # ========================================================
    ...
    # SOMENTE VELAS -2 E -3.
    #
    # A vela de confirmação NÃO entra aqui.
    # ========================================================
    ...
    if (
        ...
        # A confirmação precisa mostrar retomada.
        rompeu_pullback = (
            confirmacao["close"]
            >
            pullback_1["high"]
    ...
    if (
        ...
        # Mercado muito parado.
        if atr_ratio < 0.00008:
            atr_ok = False

        # Mercado extremamente explosivo.
        if atr_ratio > 0.0035:
            atr_ok = False
    ...
    if (
        ...
        detalhes_rejeicao = (
            "CONFIRMADA NA VELA DE SINAL"
        )

    else:

        detalhes_rejeicao = "NAO"
    ...
    if sinal == "CALL":
        ...
        mensagem = (
            "PULLBACK DE ALTA + CONFIRMACAO | "
            "Pullback ocorreu antes da vela de sinal | "
            "Vela atual confirmou retomada | "
            f"RSI={rsi14:.2f}"
        )
        ...
        mensagem = (
            ...
            f"Confirmacao={detalhes_rejeicao} | "
            f"CALL={score_call} | "
            f"PUT={score_put}"
        )
...
def enviar_sinal_telegram(
    ...
    # Só grava como enviado depois de montar
    # o sinal válido.
...
experimental e não garante resultado.
...
def health():
    ...
    return jsonify({
        ...
        "estrategia":
            "pullback + confirmacao em vela separada",
