...
def analisar(candles):
    if len(candles) < 30:
        return {
            ...
            "mensagem": "Poucas velas para analise.",
        }
    ...
    maxima = float(ultima["high"])
    minima = float(ultima["low"])
    ...
    if ema5 is not None and ema13 is not None and ema21 is not None:
        if ema5 > ema13 > ema21:
            score_call += 2
    ...
    if fechamento > abertura and forca_corpo >= 0.50:
        score_call += 1
    elif fechamento < abertura and forca_corpo >= 0.50:
        score_put += 1
    ...
    if atr14 is not None and preco != 0:
        if (atr14 / preco) < 0.00008:
            score_call = min(score_call, 2)
            score_put = min(score_put, 2)

    diferenca = abs(score_call - score_put)

    if score_call >= 4 and diferenca >= 2:
        sinal = "CALL"
        score = score_call
    elif score_put >= 4 and diferenca >= 2:
        sinal = "PUT"
        score = score_put
    else:
        sinal = "AGUARDAR"
    ...
    mensagem = f"CALL={score_call} | PUT={score_put}"
    if rsi14 is not None:
        mensagem += f" | RSI={rsi14:.2f}"
...
def processar_ativo(chave, symbol):
    try:
        ...
        if ultimo_raw is None:
            raise RuntimeError("Nao foi possivel identificar o ultimo candle.")
        ...
        estado["preco"] = (
            f"{preco:.5f}" if isinstance(preco, (float, int)) else (preco or "-")
        )
...
def executar_leitura():
    ...
    if not API_KEY:
        ...
        estado["sinal"] = "AGUARDAR"
        estado["mensagem"] = "Configure TWELVE_DATA_API_KEY no Render."
        return
...
def esperar_ate_proxima_leitura():
    ...
    log(f"Proxima leitura: {proxima.strftime('%H:%M:%S BRT')}.")
    time.sleep(segundos)
...
{{ estado.mensagem }}
<br><br>
O sinal e uma analise tecnica auxiliar e nao garante resultado.
<br>
Use primeiro em conta demo e/ou backtest.
</div>
</div>

<div class="atualizacao">
A pagina atualiza automaticamente a cada 10 segundos.
</div>
...
def health():
    return jsonify({
        ...
        "horario_brt": agora_brt().strftime("%Y-%m-%d %H:%M:%S"),
    })
